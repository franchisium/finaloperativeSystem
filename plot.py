...
for n, state in enumerate(data):
    ...
    dx = x[1] - x[0]
    psi /= np.sqrt(np.sum(psi**2) * dx)  # funcion para normalizar
    psi_squared = psi**2
    potential = 0.5 * x**2  # Potencial armonico
...
#escalar valores
    ...
    axs[n].plot(x, psi, label=r"$\Psi(x)$", linewidth=1.5)
    axs[n].plot(x, psi_squared, label=r"$|\Psi(x)|^2$", linestyle="--")
    axs[n].plot(x, potential, 'k--', label=r"$V(x)$ (escalado)", alpha=0.7)
